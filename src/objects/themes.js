export const themes = {
    light: {
        navbarBackgroundColor: '#CE0627',
        defaultFontColor: '#000000',

        dropdownLabelBackgroundColor: '#9A041D',
        dropdownHoverColor: '#9A041D',

        dropdownMenuBackgroundColor: '#FFFFFF',
        dropdownMenuRowHoverColor: '#E6E6E6',

        dropdownMenuBorder: '#9A041D',
        dropdownIconBackgroundColor: '#FFFFFF',

        dropdownMenuScrollbarTrackColor: '#f0f0f0',
        dropdownMenuScrollbarThumbColor: '#cdcdcd',
        dropdownMenuScrollbarThumbHoverColor: '#b7b6b6',
        dropdownMenuScrollbarThumbActiveColor: '#767676',

        filterModalOverlayScreen: 'rgba(0, 0, 0, 0.50)',
        filterModalBackgroundColor: 'rgba(255, 255, 255, 0.25)',
        filterModalTitleColor: '#FFFFFF',
        filterModalHeaderBorderColor: '#FFFFFF',
        filterModalFontColor: '#FFFFFF',

        themeTogglerBackgroundColor: '#9A041D',
        themeTogglerButtonColor: '#F2C03B',

        dividerColor: '#E6E6E6',

        background: '/images/light-mode-background.jpg',
        themeIcon: '/images/light-theme-icon.svg',

        panelBackgroundColor: 'rgba(255, 255, 255, 0.25)',
        panelInnerBorderColor: 'rgba(255, 255, 255, 0.25)',
        panelFontColor: '#000000',

        pokemonCardBackgroundColor: 'rgba(56, 56, 56, 0.3)',

        loadMoreCardBackgroundColor: 'rgba(56, 56, 56, 0.3)',
        loadMoreCardColor: '#FFFFFF',
        loadMoreCardHoverBackgroundColor: '#CE0627',
        loadMoreCardHoverColor: '#FFFFFF',
        loadMoreIconColor: 'rgba(0, 0, 0, 0.08)',

        scrollButtonColor: '#9C9C9C',
        scrollBackgroundColor: 'rgba(255, 255, 255, 0.4)',

        navigationTabActiveBackgroundColor: '#FFFFFF',
        navigationTabActiveColor: '#CE0627',

        navigationTabInactiveBackgroundColor: '#EEEEEE',
        navigationTabInactiveColor: '#000000',

        navigationTabHoverBackgroundColor: '#CE0627',
        navigationTabHoverColor: '#FFFFFF',

        navigationTabBodyBackgroundColor: '#FFFFFF',
        navigationTabBodyBackgroundIllustrationColor: '#EDEDED',

        tabBodyBackgroundIllustration: '/images/background-illustration.png',
        tabBodyBackgroundIllustrationMobile: '/images/background-illustration-mobile.png',

        scrollbarTrackColor: 'rgba(255, 255, 255, 0.4)',
        scrollbarThumbColor: '#9C9C9C',
        scrollbarThumbHoverColor: '#767676',
        scrollbarThumbActiveColor: '#545353',

        scrollbarTabTrackColor: 'rgba(0, 0, 0, 0.1)',

        backButtonIconBackgroundColor: '#FFFFFF',
        backButtonHoverColor: '#9A041D',       
        backButtonLabelBackgroundColor: '#9A041D'
    },
    dark: {
        navbarBackgroundColor: '#383838',
        defaultFontColor: '#FFFFFF',

        dropdownLabelBackgroundColor: '#2A2A2A',
        dropdownHoverColor: '#000000',

        dropdownMenuBackgroundColor: '#282828',
        dropdownMenuRowHoverColor: '#606060',

        dropdownMenuBorder: '#2A2A2A',
        dropdownIconBackgroundColor: '#FFFFFF',

        dropdownMenuScrollbarTrackColor: 'rgba(0, 0, 0, 0.4)',
        dropdownMenuScrollbarThumbColor: '#cdcdcd',
        dropdownMenuScrollbarThumbHoverColor: '#b7b6b6',
        dropdownMenuScrollbarThumbActiveColor: '#767676',

        filterModalOverlayScreen: 'rgba(0, 0, 0, 0.50)',
        filterModalBackgroundColor: 'rgba(0, 0, 0, 0.25)',
        filterModalTitleColor: '#FFFFFF',
        filterModalHeaderBorderColor: '#FFFFFF',
        filterModalFontColor: '#FFFFFF',

        themeTogglerBackgroundColor: '#2A2A2A',
        themeTogglerButtonColor: '#25AFEE',

        dividerColor: 'rgba(255, 255, 255, 0.1)',

        background: '/images/dark-mode-background.jpg',
        themeIcon: '/images/dark-theme-icon.svg',

        panelBackgroundColor: 'rgba(0, 0, 0, 0.25)',
        panelInnerBorderColor: 'rgba(0, 0, 0, 0.15)',
        panelFontColor: '#FFFFFF',

        pokemonCardBackgroundColor: 'rgba(255, 255, 255, 0.15)',

        loadMoreCardBackgroundColor: 'rgba(0, 0, 0, 0.25)',
        loadMoreCardColor: '#FFFFFF',
        loadMoreCardHoverBackgroundColor: '#383838', 
        loadMoreCardHoverColor: '#FFFFFF',
        loadMoreIconColor: 'rgba(255, 255, 255, 0.3)',

        scrollButtonColor: '#9C9C9C',
        scrollBackgroundColor: 'rgba(0, 0, 0, 0.4)',      
        
        navigationTabActiveBackgroundColor: '#282828',
        navigationTabActiveColor: '#FFFFFF',
        navigationTabInactiveBackgroundColor: '#606060',
        navigationTabInactiveColor: '#AFAFAF',
        navigationTabBodyBackgroundColor: '#282828',
        navigationTabBodyBackgroundIllustrationColor: '#3A3A3A',

        navigationTabHoverBackgroundColor: '#FFFFFF',
        navigationTabHoverColor: '#383838',

        tabBodyBackgroundIllustration: '/images/background-illustration-dark.png',
        tabBodyBackgroundIllustrationMobile: '/images/background-illustration-mobile-dark.png',

        scrollbarTrackColor: 'rgba(0, 0, 0, 0.4);',
        scrollbarThumbColor: '#9C9C9C',
        scrollbarThumbHoverColor: '#CACACA',
        scrollbarThumbActiveColor: '#FFFFFF',

        backButtonIconBackgroundColor: '#FFFFFF',
        backButtonHoverColor: '#000000',       
        backButtonLabelBackgroundColor: '#2A2A2A'
    } 
}